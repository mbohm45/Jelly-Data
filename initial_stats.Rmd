...
# getting rid of polyp jar 11, as it is an outlier in buds and prevents model from running (see model below that does not converge)
...
library(dplyr)
polyp_buds <- polyp_clean %>%
  slice(-c(35))

# buds: generalized beta 1 (AIC = -50.91) - still does not converge
fitDist(prop_buds, data = polyp_buds, type = "real0to1", try.gamlss = T)
mod_buds <- gamlss(prop_buds ~ treatment*collection_day + re(random = ~1 | jar_code), family = GB1(), data = polyp_buds, control = gamlss.control(n.cyc = 250))
histDist(polyp_buds$prop_buds, "GB1", density = T, main = "Gen Beta 1")
...
# p-value = 0.117, so not significant
...
summary(mod_ruffled)
# p-value = 6.99e-05, so significant!
