...
e_GB1 <- gamlss(prop_elongated ~ treatment*collection_day + re(random = ~1 | jar_code), family =GB1(), data = polyp_clean)

# ruffled: logit normal (AIC = -9.42)
fitDist(prop_ruffled, data = polyp_clean, type = "real0to1", try.gamlss = T)
r_LOGITNO <- gamlss(prop_ruffled ~ treatment*collection_day + re(random = ~1 | jar_code), family = LOGITNO(), data = polyp_clean)

# buds: generalized beta 1 (AIC = -41.12)
fitDist(prop_buds, data = polyp_clean, type = "real0to1", try.gamlss = T)
b_GB1 <- gamlss(prop_ruffled ~ treatment*collection_day + re(random = ~1 | jar_code), family = LOGITNO(), data = polyp_clean)
