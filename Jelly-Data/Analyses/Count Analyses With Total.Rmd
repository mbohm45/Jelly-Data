...
polyp_clean <- polyp_data %>%
  ...
  mutate(prop_elongated = num_elongated/total_num + 0.001, 
         ...
         log_elongated = log(num_elongated+1))  %>%
  dplyr::select(collection_day, jar_code, treatment, num_elongated, prop_elongated, num_ruffled, prop_ruffled, num_asexual_buds, prop_buds, total_num, log_elongated) 
...
fitDist(log_elongated, data = polyp_clean, try.gamlss = T)
```
...
"Our model suggests that caffeine has a statistically signifcant impact on the variation in elongation (p = _____), but collection day does not (p = ____)(Fig. 1, Table S1) "
...
histDist(polyp_clean$log_elongated, 
         "SEP2", 
         density = T, 
         main = "Count Polyp Elongation Compared to Skewed Exp II",
         xlab = "Number Elongated")
```
